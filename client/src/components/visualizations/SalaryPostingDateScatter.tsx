    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      <div className="p-4 border-b border-gray-200">
        <h2 className="text-lg font-semibold flex items-center">
          <span className="material-icons text-primary mr-2">scatter_plot</span>
          Correlation Between Salary and Posting Date
        </h2>
        <p className="text-sm text-gray-500">Scatter plot with trend line showing salary trends over time</p>
      </div>
      <div className="p-4">
        <div className="flex items-center justify-end mb-4 space-x-2">
          <button 
            className={`text-xs ${showTrendLine ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700'} py-1 px-2 rounded hover:bg-gray-200 transition`}
            onClick={() => setShowTrendLine(!showTrendLine)}
          >
            Show Trend Line
          </button>
          <button 
            className={`text-xs ${groupByIndustry ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700'} py-1 px-2 rounded hover:bg-gray-200 transition`}
